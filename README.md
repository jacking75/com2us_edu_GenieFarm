...
우선 로그인 시도시 현재 앱 버전 및 마스터데이터 버전을 우선 검증한다.

계정 인증을 플랫폼에서 따로 할 경우

- 계정ID만 받아온다.

계정 인증부터 게임 자체에서 할 경우

- 입력된 아이디와 패스워드를 검증한 후
    - 아이디 중복체크
    - 패스워드 보안
- 아이디에 대응되는 계정ID를 로딩한다.

이후 계정ID에 대한 인증키를 생성하여 Redis에 저장한다. 생성된 인증키는 로그인 이후 요청시의 인증에 사용한다.

단, 이 때 이미 해당 계정ID에 대한 인증키가 존재할경우 중복 로그인으로 취급하여 인증키를 삭제한다.

인증키가 정상적으로 발급되었다면 로그인 성공으로 취급하여 유저 정보를 로딩한다.

계정ID에 해당하는 유저 정보가 존재할 경우에는 기존 유저이므로 기존 유저 데이터를 로딩한다.

유저 정보가 존재하지 않을 경우에는 신규 유저이므로 유저 기본 데이터를 생성한다.

- 닉네임 설정(중복 검증 필요)
- 유저 정보(레벨, 경험치, 기본아이템, 능력치, 재화 등)
- 기본 농장 정보(농장 크기, 배치된 건물 등)

로그아웃시에는 Redis에 저장해둔 인증키를 삭제한다.

---

- 로그인
    - 앱 버전, 마스터데이터 버전 확인
    - Redis에 토큰 저장해서 인증 처리
    - 로그인 성공 시, 데이터 로드로 연계되도록
- 로그아웃
    - Redis 토큰 삭제
- 유저 데이터 로딩
    - 유저 기본 정보, 농장 기본 정보 등 로그인시 곧바로 보이거나 활용되어야 하는 데이터들만 로딩한다.
    - 우편함, 친구 등의 데이터는 해당 항목 접근시에 로딩한다.

## 출석

매달 1일을 기준으로 초기화된다.

총 28일치의 보상이 있으며, 월간 누적 출석일수를 기준으로 보상을 지급한다(연속적으로 출석하지 않아도 누적수는 변하지 않는다).

주말에는 2일치의 출석을 인정해준다.

월간 구독제를 이용하고 있는 유저라면, 별도의 특별 보상을 추가로 받을 수 있다.

- 월간 누적 출석일수와는 완전히 별개이다.
- 달이 지나도 초기화되지 않고, 월간 구독제의 남은 차감횟수에 따라서만 지급한다.

---

- 출석 체크(체크와 함께 보상 수령)
    - 출석부에서 버튼을 눌러야 체크가 된다(로그인과 동시에 처리되지는 않는 시스템).
    - 보상은 우편함으로 지급된다.
